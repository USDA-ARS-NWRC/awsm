...
with awsm.framework.framework.AWSM(configFile) as a:
    ...
    for tp in tps:
        for sn in scen_run:
            ...
            a.project_description = 'AWSM run for {} scenario {}'.format(
                tp, sn)
            ...
            csv_p = '/data/blizzard/awsftest/febWeather/{}/scenario_{}/'.format(
                tp, sn)

            a.config['csv']['metadata'] = os.path.join(csv_p, 'metadata.csv')
            a.config['csv']['air_temp'] = os.path.join(csv_p, 'air_temp.csv')
            a.config['csv']['vapor_pressure'] = os.path.join(
                csv_p, 'vapor_pressure.csv')
            a.config['csv']['precip'] = os.path.join(
                csv_p, 'precip_intensity.csv')
            a.config['csv']['solar'] = os.path.join(
                csv_p, 'solar_radiation.csv')
            a.config['csv']['wind_speed'] = os.path.join(
                csv_p, 'wind_speed.csv')
            a.config['csv']['wind_direction'] = os.path.join(
                csv_p, 'wind_direction.csv')
            a.config['csv']['cloud_factor'] = os.path.join(
                csv_p, 'cloud_factor.csv')
            ...
            a._logger.info('Scenario {} ran in {}'.format(
                sn, datetime.now() - start_temp))
